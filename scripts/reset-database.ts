 ...
 *   npx ts-node scripts/reset-database.ts --seed
 *   npx ts-node scripts/reset-database.ts --confirm
 *   npx ts-node scripts/reset-database.ts --confirm --seed
 */
...
async function resetDatabase(shouldSeed: boolean = false) {
  ...
  if (!args.includes('--confirm')) {
    ...
    console.log('   Example: npx ts-node scripts/reset-database.ts --confirm')
    console.log('')
    process.exit(0)
  }
  ...
  try {
    ...
    // Optional seeding
    if (shouldSeed || args.includes('--seed')) {
      await seedDemoData()
    }
...
async function seedDemoData() {
  ...
  const hashedJudgeCode = await hash(judgeCode, 10)
  ...
  const hashedJudgeCode2 = await hash(judgeCode2, 10)
  await prisma.judge.create({
    data: {
      eventId: event.id,
      name: 'Second Judge',
